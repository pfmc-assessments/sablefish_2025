...
![Estimated yield curve with reference points for the base model. The biomass target (Btgt) and the spawning potential ratio (SPR) target both are maximized at 0.40 fration unfished.](figures/yield2_yield_curve_with_refpoints.png){#fig-es-yield height=3in fig-alt=""}
...
```{r, results = "asis"}
#| label: tbl-es-decision-table-pstar-45
#| warning: false
#| echo: false
#| tbl-cap: ""
#| tbl-pos: H

 load(file = here::here("report", "rda", "pstar_45.rda"))

 catch_stream |>
  dplyr::rename(
    `Low SO` = Low_SO,
    `Low Stock Status` = Low_depl,
    `Stock Status` = depl,
    `High SO` = High_SO,
    `High Stock Status` = High_depl
  ) |>
  gt::gt() |>
  gt::fmt_number(
    columns = c(2:3, 5, 7),
...
```{r, results = "asis"}
#| label: tbl-es-decision-table-pstar-40
#| warning: false
#| echo: false
#| tbl-cap: ""
#| tbl-pos: H

 load(file = here::here("report", "rda", "pstar_40.rda"))
...
risk |>
  ...
  gt::tab_style(
    style = gt::cell_borders(sides = c("top", "bottom"),  color = "grey", weight = "3pt", style = "solid"),
    locations = gt::cells_body()
  ) |>
  gt::cols_label(
