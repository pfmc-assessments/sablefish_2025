...
#' plot_fleet_selectivity(model_out, model_dir, fleet_num)
#' 
#' 
...
plot_fleet_selectivity <- function(model_out, model_path, fleet_num){
    ...
    plot <- ggplot2::ggplot(data, aes(x=age, y=sel, color=factor(timeblock), linetype=sex, shape=sex, group=interaction(sex, timeblock)))+
        ...
        ggplot2::labs(x="Age", y="Selectivity", color="Time Block", linetype="Sex", shape="Sex")+
        ggplot2::guides(shape="none", linetype="none")+
        ggplot2::scale_y_continuous(limits=c(0, 1), breaks=seq(0,1, 0.25), labels=seq(0,1,0.25))+
...
#' fleet_num <- 1 
#' mod_out <- r4ss::SS_output(dir=model_dir)
#' plot_fleet_retention(model_out, model_dir, fleet_num)
#' 
...
plot_fleet_retention <- function(model_out, model_path, fleet_num){
    ...
    plot <- ggplot2::ggplot(data, aes(x=age, y=ret, color=factor(timeblock), linetype=sex, shape=sex, group=interaction(sex, timeblock)))+
        ggplot2::geom_line()+
        ggplot2::geom_point(size=2)+
        ggplot2::labs(x="Age", y="Retention", color="Time Block", linetype="Sex", shape="Sex")+
        ggplot2::guides(shape="none", linetype="none")+
        ggplot2::scale_y_continuous(limits=c(0, 1), breaks=seq(0,1, 0.25), labels=seq(0,1,0.25))+
        ggplot2::scale_shape_manual(values=c(16, 1))+
        ggplot2::facet_wrap(~Fleet)+
