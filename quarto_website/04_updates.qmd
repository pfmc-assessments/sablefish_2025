...
The other factor that had led to the stock assessment team modifying the catch streams has been driven by model stability and transparency.  Groundfish assessments have applied one of the two following approaches to estimate total mortality (landings plus dead discards): 1) estimate selectivity and retention with assumptions about discard mortality within the model using available discard data (discard rates and lengths or ages of discarded fish) or 2) include separate landings and discard fleets in the model with discard mortality being estimated external to the model. These two approaches, when the models are parameterized in a comparable fashion, should result in similar to nearly identical model estimates. Previous sablefish assessments have opted to estimate selectivity and retention within the model. However, during model development the stock assessment team determined that this model structure appeared to be unstable, created challenges in estimating discard mortality that aligned with estimates by gear with the Groundfish Expanded Multiyear Mortality report, and was overall less transparent when the model was producing reasonable estimates of selectivity and retention. Given these challenges, the stock assessment team, opted to move to a model with separate discard fleets.  
