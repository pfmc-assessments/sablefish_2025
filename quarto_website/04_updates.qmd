...
Since the pre-assessment data workshop, the stock assessment team has modified the landings and the proposed model structure to correct incorrect assumptions from previous assessments and to support a more stable model. Previous assessments of West Coast sablefish have added historical foreign fleet removals current gear-specific landings fleets with the model estimating additional mortality from discarding (i.e., using estimates of retention and the mortality of discarded sablefish by gear type).  When reviewing the historical documentation that provides estimates of historical removals by foreign fleets in U.S. waters, it was determined that the reported totals by gear type and year represented total catches (landings plus discards) meaning it would be inaccurate to estimate additional discard mortality from these catch totals which was done in both the 2011 and 2019 benchmark assessments.  

The other factor that had led to the stock assessment team modifying the catch streams has been driven by model stability and transparency.  Groundfish assessment had applied one of the two following approaches to estimate total mortality (landings plus dead discards): 1) estimate selectivity and retention with assumptions about discard mortality within the model using available discard data (discard rates and lengths or ages of discarded fish) or 2) include separate landings and discard fleets in the model with discard mortality being estimated external to the model. These two approaches, when the models are parameterized in a comparable fashion, should result in similar to nearly identical estimates. Previous sablefish assessments have opted to estimate selectivity and retention within the model. However, during model development the stock assessment team determined that this model structure appearred to be unstable, created challenges in estimating discard mortality that aligned with estimates by gear with the Groundfish Expanded Multiyear Mortality 
report, and was overall less transparent when the model was producing reasonable estimates of selectivity and retention.  Given these challenges, the stock assessment team, opted to move to a model with separate discard fleets.  

The new fleets used in the model will increase from three (trawl, hook & line, and pot) to a total of six fishery fleets with the additional three fleets that represent estimated discard mortality.  The discard mortality was estimated based upon the gear using historical discard studies (e.g., Pikitch) and current discard information collected by the West Coast Groundfish Observer Program (WCGOP). In summary, the proposed fleets are now:
...
Since the pre-assessment data workshop, the stock assessment team investigated the data from the AFSC Slope survey in depth.  During these investigations it was determined that aged fish for sablefish did not appear to be a representative sample of the lengths
observed. A similar issue for the biological data collected by this survey was also observed for Dover sole. Additionally, this survey had an unusually high rate of positive tows for sablefish (e.g., ~ 98% of tows observed sablefish), particularly compared to the NWFSC Slope survey (~ 86%) that was conducted across similar years and depths. Given the concerns about the data collected by the AFSC Slope survey for sablefish, and the fact that there was a comparable survey data set during these years, the stock assessment team decided to not include data from the AFSC Slope survey in this assessment.
