...
#| echo: false
#| warning: false
load(file.path(here::here(), "data", "data_commercial_bds.rda"))
bds_data <- data_commercial_bds %>% as_tibble() %>%
  mutate(
    area = case_when(
...
Length samples are generally available for all gear types across the three states. There has been a substantial decline in length sampling in California in recent years, and a total lack of length sampling of trawl catch in Southern California since 2012. Length sample availability in Oregon and Washington has remained stable since the 2010s.
```{r}
#| label: nsamples-length
#| echo: false
#| warning: false
#| fig-cap: "Number of available length samples over time, grouped by gear type and state."
#| fig-cap-location: margin
...
Age samples are generally available from Oregon and Washington state, but have not been available from Northern California since 2018 or from Southern California since 2003. 
```{r}
#| label: nsamples-age
#| echo: false
#| warning: false
...
Commercial length compositions from trawl fisheries are similar across the coast. Meanwhile, length compositions from the hook-and-line and pot fisheries are skewed towards larger fish in Oregon and Washington as compared to California. Length compositions of hook-and-line and pot catch are often similar, though the pot fishery may catch slightly smaller fish.
```{r}
#| label: length-by-gear-region
#| echo: false
#| warning: false
#| fig-cap: "Distribution of length samples by gear type and state."
#| fig-cap-location: margin

bds_data %>%
  group_by(area, geargroup) %>%
  select(sex, length_cm) %>%
  mutate(area=factor(area)) %>%

  ggplot(aes(x=length_cm, fill=geargroup))+
    ggridges::geom_density_ridges(aes(y=area), alpha=0.5, scale=1)+
    labs(y="Area", x="Length (cm)", fill="Gear")+
...
#| echo: false
#| warning: false
#| fig-cap: "Distribution of age samples by gear type and state."
#| fig-cap-location: margin

bds_data %>%
  group_by(area, geargroup) %>%
  select(sex, age_years) %>%
  mutate(area=factor(area)) %>%

  ggplot(aes(x=age_years, fill=geargroup))+
    ggridges::geom_density_ridges(aes(y=area), alpha=0.5, scale=1)+
    labs(y="Area", x="Age", fill="Gear")+
    scale_x_continuous(limits=c(0, 125), expand=c(0, 0))+
    scale_fill_viridis_d()+
    theme_bw()
```
...
#| echo: false
#| warning: false
#| fig-cap: "Distribution of length samples by gear and region for three time periods. Time periods are generally reflective of known changes in either selectivity or retention."
#| fig-cap-location: margin

ggplot(bds_data,
       aes(x = length_cm, y=period, fill = geargroup, alpha = 0.25)) +
...
#| echo: false
#| warning: false
#| fig-cap: "Distribution of age samples by gear and region for three time periods. Time periods are generally reflective of known changes in either selectivity or retention."
#| fig-cap-location: margin

ggplot(bds_data,
       aes(x = age_years, y=period, fill = geargroup, alpha = 0.25)) +
  ggridges::geom_density_ridges(scale=0.9) +
  scale_x_continuous(limits=c(0, 125), expand=c(0, 0), breaks=seq(0, 125, 25))+
  labs(y="Time Period", x="Age", fill="Gear Type")+
  guides(alpha="none")+
  facet_wrap(~area, ncol=2) +
  scale_fill_viridis_d()+
  theme_bw()+
